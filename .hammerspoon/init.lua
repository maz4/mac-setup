...
function cycleWindowsOfSameApp()
	...
	if focusedApp then
		...
		end) -- Filter only visible windows
...
-- hotkey.bind({ "cmd" }, "2", focusOn("Cursor"))
-- hotkey.bind({"cmd"}, "2", focusOn("Code"))
hotkey.bind({ "cmd" }, "2", focusOn("Zed"))
-- hotkey.bind({ "cmd" }, "2", focusOn("Intellij IDEA"))

-- Binding Cmd + 3 to focus Ghostty
hotkey.bind({ "cmd" }, "3", focusOn("Ghostty"))

-- Binding Cmd + 4 to focus
hotkey.bind({ "cmd" }, "4", focusOn("Preview"))

-- Binding Cmd + 5 to focus Notion
hotkey.bind({ "cmd" }, "5", focusOn("Obsidian"))
